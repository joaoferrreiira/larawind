<x-app-layout title="Dicas">
    <div class="container grid px-6 mx-auto">
        <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            @if($edit) Editar @else Adicionar @endif dica
        </h2>

        <form method="POST" @if($edit) action="/dicas/{{$tips->id}}" @else action="/dicas" @endif autocomplete="off">
            {{ csrf_field() }}
            @if($edit) {{ method_field('PUT') }} @endif
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <label class="block text-sm">
                    <span class="text-gray-700 dark:text-gray-400">Dica</span>
                    <input @if($edit) value="{{$tips->tip}}" @else value="" @endif name="dica" required type="text" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-green-400 focus:outline-none focus:shadow-outline-green dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="Dica" autocomplete="off"/>
                </label>
